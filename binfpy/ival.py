...
class IntervalTree:
    ...
    def isect(self, ival, node=None):
        ...
        if self.root == None:
            return None
        if node == None:
            return self.isect(ival, self.root)
        while node != None:
            if isect(ival, node.ival):
                return node
            elif node.left == None:
                node = node.right
            elif node.left.max < ival.min:
                node = node.right
            else:
                node = node.left
        return None
    ...
    def _randomizedInsert(self, node, ival, value):
        if node == None:
            return IntervalNode(ival, value)
        if random.uniform(0, 1) * node.N < 1.0:
            return self._rootInsert(node, ival, value)
        if ival < node.ival:
...
def _closest(query, cand):
    ...
    while cand != None:
        ...
        if cand.left == None:
            cand = cand.right
        elif cand.right == None:
            cand = cand.left
        elif cand.ival.min > query.max:
            cand = (
                cand.left
            )  # the smallest, indexed value (on left) is AFTER the query min
...
class Interval:
    ...
    def __lt__(self, other):
        ...
        return (
            True
            if (
                self.min < other.min or (self.min == other.min and self.max < other.max)
            )
            else False
        )
    ...
    def dist(self, that, signed=False, centre2centre=False):
        ...
        if not centre2centre:
            if not signed:
                if self.min > that.max:
                    return self.min - that.max  # that interval is BEFORE this
                if self.max < that.min:
                    return that.min - self.max  # that interval is AFTER this
            else:  # distance is signed
                if self.min > that.max:
                    return that.max - self.min  # that interval is BEFORE this
                if self.max < that.min:
                    return that.min - self.max  # that interval is AFTER this
            return 0
        else:
