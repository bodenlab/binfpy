...
def countWordsReport(seqs, WordWidth=8, PeakWidth=100, PeakMargin=100):
    ...
    for seq in seqs:
        ...
        words = set(
            slidewin(seq[centre - PeakWidth / 2 : centre + PeakWidth / 2], WordWidth)
        )
        for word in words:
            try:
                pos[word] += 1
    ...
    for word, lgr in sortpos[
        ...
        pval = stats.getFETpval(
            ...
            len(seqs) * (len(seq) - (PeakMargin * 2 + PeakWidth) - (WordWidth - 1) * 2)
            - cnt_neg,
            False,
        )
...
def getReverse(distribs):
    ...
    return [
        d.swapxcopy("A", "T").swapxcopy("C", "G") for d in distribs[::-1]
    ]  # backwards
...
def scanMotifReport_new(
    seqs, motif, threshold=3.4567, jaspar="JASPAR_matrices.txt", seed=0
):
    """Produce a plot for a scan of the specified motif.
    The plot has as its x-axis position of sequence, and
    ...
    print(
        "Number of sequences with hit (score >= %f): %d"
        % (threshold, n_seqs_with_hits),
        file=sys.stderr,
    )
    ...
    for i in range(1, (seq_len - pwm1.length / 2 + 1) / 2):
        cum_hit_count[i] = (
            cum_hit_count[i - 1] + hit_count[center - i] + hit_count[center + i]
        )
    ...
    for i in range(hw, seq_len - motif_width + 1 - hw):
        smoothed_site_probability[i] = sum(site_probability[i - hw : i + hw + 1]) / (
            2 * hw + 1
        )
...
if __name__ == "__main__":
    ...
    if DISCOVER_MODE:
        print(
            ...
            % (FILENAME, WORD_WIDTH, PEAK_WIDTH, PEAK_MARGIN)
        )
