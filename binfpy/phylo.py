...
import binfpy.annotations as annotations
...
class PhyloTree:
    ...
    def putAnnotations(self, nexus_annotations):
        ...
        for node in self.getNodes():
            if node.label in self.nexus_annotations.leaf_annotations:
                self.nexus_annotations.leaf_annotations[node] = (
                    self.nexus_annotations.leaf_annotations[node.label]
                )
    ...
    def swap_annotations(self, annotation_key):
        try:
            for node in self.getNodes():
                if node.isLeaf():
                    node.label = self.nexus_annotations.leaf_annotations[node][
                        annotation_key
                    ]
        except:
            return
        ...
        if nexus_annotations:
            for node in self.getNodes():
                if node in self.nexus_annotations.node_annotations:
                    node.annotate_node(
                        self.nexus_annotations.node_annotations,
                        self.nexus_annotations.annotation_symbols,
                        exclude_annotations,
                        use_symbols,
                    )
            ...
            for node in self.getNodes():
                if node in self.nexus_annotations.leaf_annotations:
                    node.annotate_node(
                        self.nexus_annotations.leaf_annotations, exclude_annotations
                    )
                leaves = []
                for node in self.getNodes():
                    if node.isLeaf():
...
class PhyloNode:
    ...
    def _propagateDistance(self, parent_dist):
        ...
        for i in range(self.nChildren()):
            self.children[i]._propagateDistance(
                travelled
    ...
    def _forwardParsimony(self, aln):
        ...
        if self.sequence == None:  # no sequence has been assigned
            ...
            self.backptr = [
                [[None for _ in aln.alphabet] for _ in range(aln.alignlen)]
            ...
            self.seqscores = [
                [0 if a == sym else 999999 for a in aln.alphabet]
                for sym in self.sequence
            ]  # if we want to weight scores, this would need to change
        if self._verbose:
            print("Forward:", self.label, "\n\t", self.seqscores)
        ...
        if self.sequence == None:  # no sequence has been assigned
            ...
            if (
                ...
                self.sequence = sequence.Sequence(
                    currbuf, aln.alphabet, self.label, gappy=True
                )
            ...
            for i in range(self.nChildren()):
                self.children[i]._backwardParsimony(
                    ...
                    sequence.Sequence(
                        childbuf[i],
                        aln.alphabet,
                        self.children[i].label or "Child of " + self.label,
                        gappy=True,
                    ),
                )
        if self._verbose:
                ...
                for val in val_list:
                    if use_symbols and val in annotation_symbols:
                        sorted_symbols = sorted(
                            [annotation_symbols[val] for val in val_list]
                        )
                        annotation_string += '%s="%s",' % (
                            key,
                            " ".join(["%s" % (val,) for val in sorted_symbols]),
                        )
                    else:
                        annotation_string += '%s="%s",' % (
                            key,
...
def runUPGMA(aln, measure, absoluteDistances=False):
    ...
    while (
        len(N) > 1
    ):  # N will contain all "live" clusters, to be reduced to a single below
        closest_pair = (
            None,
            None,
...
if __name__ == "__main__":
    aln = sequence.readFastaFile(
        "/Users/mikael/Documents/Teaching/SCIE2100/2020/dnaexamq.aln",
        sequence.Protein_Alphabet,
    )
    tree = runUPGMA(sequence.Alignment(aln), "fractional")
    writeNewickFile(
        "/Users/mikael/Documents/Teaching/SCIE2100/2021/examq_pdist.nwk", tree
    )

if __name__ == "__main__3":
    aln = sequence.readClustalFile(
        "/Users/mikael/simhome/ASR/dp16_example.aln", sequence.Protein_Alphabet
    )
    ...
    tree.putAlignment(
        sequence.Alignment(
            sequence.readFastaFile(
                "/Users/mikael/simhome/ASR/parsitest2.aln", sequence.DNA_Alphabet
            )
        )
    )
    tree.parsimony()
