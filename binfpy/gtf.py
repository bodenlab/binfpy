    ...
    def __contains__(self, item):
        if isinstance(item, GtfEntry):
            ...
            if tree == None:
                ...
                return ival.Interval(item.start, item.end) in tree
        else:
            return False
    ...
    def getIndex(self, group_attribute):
        ...
        for entry in self:
            if group_attribute in entry.attr:  # is the entry indexed with the attribute
                if (
                    not entry.attr[group_attribute] in d
                ):  # only add the first entry for this particular value
    ...
    for line in f:
        ...
        try:
            ...
            if len(words) > 8:
                group = words[8]
            entry = GtfEntry(
                seqname, start, end, feature, score, source, strand, frame, group
            )
            # check if the chromosome has been seen before
            ...
            if not acceptHeaderRows:
                raise RuntimeError(
                    "Error in GTF/GFF file at row %d (%s)" % (row, e.strerror)
                )
            else:
                headerRow = words
                acceptHeaderRows -= (
