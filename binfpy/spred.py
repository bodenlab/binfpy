...
class SeqNN:
    ...
    def _encodeseq(self, seqs, targets=None):
        ...
        for i in range(len(seqs)):
            ...
            for k in range(len(subseqs)):
                ...
                if targets:
                    om[row, self.outp_alpha.index(subtarg[k])] = 1
                row += 1
    ...
    def observeAll(self, seqs, targets, eta=0.1, niter=1):
        ...
        assert len(seqs) == len(
            targets
        ), "Number of input sequences need to match the number of target sequences"
        ...
        for seq in seqs:  # ... based on AA sequences
            nn1seq = self.predict(
                seq, useCascade=False
            )  # construct a new sequence which consists of SS predictions
            nn1seqs.append(nn1seq)
        im, om = self._encodeseq(
            nn1seqs, targets
    ...
    def testAll(self, seqs, targets):
        ...
        assert len(seqs) == len(
            targets
        ), "Number of input sequences needs to match the number of target sequences"
        if not self.cascade:
            im, om = self._encodeseq(seqs, targets)
    ...
    def predict(self, inpseq, useCascade=True):
        ...
        if useCascade and self.cascade:
            ...
            predsyms = [
                "C" for _ in range(len(inpseq))
            ]  # use coil for positions in flanking regions
            for i in range(
                len(subseqs)
            ):  # for each input sub-sequence of the primary NN
