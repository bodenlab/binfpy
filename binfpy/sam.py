...
def PercentReadsAligned(group1, group2, group3, numfastq):
    ...
    x = [
        len(group1) / float(numfastq),
        len(group2) / float(numfastq),
        len(group3) / float(numfastq),
        Unmapped,
    ]
...
def plot_base_composition(reads, sym):
    ...
    groups = [
        list(map(operator.itemgetter(1), list(group)))
        for key, group in itertools.groupby(all_items, operator.itemgetter(0))
    ]
    for group in groups:
...
def write_RPKM_data(RPKM_data, filename):
    ...
    for i in range(0, len(RPKM_data)):
        f.write(
            "%s\t%d\t%d\t%d\t%d\n"
            % (
                list(RPKM_data.keys())[i],
                int(list(RPKM_data.values())[i][0]),
                int(list(RPKM_data.values())[i][1]),
                int(list(RPKM_data.values())[i][2]),
                int(list(RPKM_data.values())[i][3]),
            )
        )
    f.close()
...
def plotMAreprpkm(rpkm_data, timepoint):
    ...
    if timepoint == "t1":
        for i in range(0, len(list(rpkm_data.values()))):
            y = np.log2(list(rpkm_data.values())[i][0] + 1) - np.log2(
                list(rpkm_data.values())[i][1] + 1
            )
            x = (
                np.log2(list(rpkm_data.values())[i][0] + 1)
                + np.log2(list(rpkm_data.values())[i][1] + 1)
            ) / 2
            m.append(y)
            a.append(x)
    else:
        for i in range(0, len(list(rpkm_data.values()))):
            y = np.log2(list(rpkm_data.values())[i][2] + 1) - np.log2(
                list(rpkm_data.values())[i][3] + 1
            )
            x = (
                np.log2(list(rpkm_data.values())[i][2] + 1)
                + np.log2(list(rpkm_data.values())[i][3] + 1)
            ) / 2
            m.append(y)
            a.append(x)
    plt.figure(1, figsize=(8, 8))
    ...
    plt.plot(a, m, "o")
    plt.axhline(
        np.mean(m) + 1.96 * np.std(m), color="green", label="avg diff +1.96(std diff)"
    )
    plt.axhline(
        np.mean(m) - 1.96 * np.std(m), color="green", label="avg diff -1.96(std diff)"
    )
...
def Welcht(rpkm):
    ...
    for i, ii, s, ss in list(rpkm.values()):
        sd1 = np.std([i, ii])
        sd2 = np.std([s, ss])
        t = (np.mean([s, ss]) - np.mean([i, ii])) / (
            math.sqrt(((float(sd2) / 2) + (float(sd1) / 2)))
        )
        ts.append(t)
    pvals = []
    for t in ts:
        ...
        if pval == float("nan"):
            ...
            pval = pval
            pvals.append(pval)
    corr_pvals = correct_pvalues_for_multiple_testing(
        pvals, correction_type="Benjamini-Hochberg"
    )
    for i in range(0, len(list(rpkm.values()))):
        result[list(rpkm.keys())[i]] = [
            list(rpkm.values())[i][0],
            list(rpkm.values())[i][1],
            list(rpkm.values())[i][2],
            list(rpkm.values())[i][3],
            corr_pvals[i],
        ]
    return result
...
def cluster_data(data_matrix, genenames, timepoint):
    ...
    for i in range(np.shape(data_matrix)[0]):
        for j in range(1):
            D[i, j] = (
                abs(data_matrix[i] - data_matrix[j]) ** 2
            )  # use Wards method (other methods could be implemented here)
    ...
    dend = sch.dendrogram(
        linked, orientation="right", labels=labels
    )  # sets the oirentation root at the right
    plt.title(timepoint)
