...
class GO:
    ...
    children = (
        {}
    )  # redundant, parent-to-child structure: children[term] = set((term, rel))

    def __init__(
        self,
        annotFile,
        obofile,
        annotfile_columns=(1, 2, 3, 4, 6, 8),
        include_genes=None,
    ):
        ...
        for line in src:
            ...
            if line.startswith("!"):
                continue
            (gene, symb, qual, term, evid, onto, taxa) = _extractAnnotFields(
                line, annotfile_columns
            )
    ...
    def _makeIntoList(self, id_or_ids):
        if (
            type(id_or_ids) != list
            and type(id_or_ids) != set
            and type(id_or_ids) != tuple
        ):
            return [id_or_ids]
        return id_or_ids
    ...
    def getTerms(self, genes_or_gene, evid=None, onto=None, include_more_general=True):
        ...
        """
        if (
            type(genes_or_gene) != list
            and type(genes_or_gene) != set
            and type(genes_or_gene) != tuple
        ):
    ...
    def getGenes(
        self, terms_or_term, evid=None, taxa=None, rel=None, include_more_specific=False
    ):
        ...
        """
        if (
            type(terms_or_term) != list
            and type(terms_or_term) != set
            and type(terms_or_term) != tuple
        ):
            return self.getGenes4Term(
                terms_or_term, evid, taxa, rel, include_more_specific
            )
        else:
            return self.getGenes4Terms(
                terms_or_term, evid, taxa, rel, include_more_specific
            )

    def getGenes4Terms(
        self, terms, evid=None, taxa=None, rel=None, include_more_specific=False
    ):
        ...
        for term in terms:
            gomap[term] = self.getGenes4Term(
                term, evid, taxa, rel, include_more_specific
            )
        return gomap

    def getGenes4Term(
        self, term, evid=None, taxa=None, rel=None, include_more_specific=False
    ):
        """Retrieve all genes that are annotated with specified term or terms,
        qualified by evidence, taxa and relation type, e.g. "is_a".
        With a single term provided, a set of genes is returned.
    ...
    def _getGenes4Term(self, term, evid=None, taxa=None, rel=None):
        ...
        for (
            ...
            self.annots
        ):  # annotations: annots[gene] = (taxa, terms[term] = (evid, T/F))
    ...
    def getAllBackground(
        self, positives=[], taxa=None, evid=None, include_more_general=False
    ):
    ...
    def getCountReport(self, positives, threshold=None, include_more_general=True):
        ...
        fg_map = self.getTerms4Genes(
            positives, include_more_general=include_more_general
        )  #
        for id in fg_map:
            for t in fg_map[id]:
                fg_list.append(t)
    ...
    def getEnrichmentReport(
        self,
        positives,
        background=None,
        evid=None,
        threshold=None,
        include_more_general=True,
    ):
        ...
        fg_list = []  # all terms, with multiple copies for counting
        fg_map = self.getTerms4Genes(
            positives, evid=evid, include_more_general=include_more_general
        )  #
        ...
        negatives = set(background).difference(
            set(positives)
        )  # remove the positives from the background to create genuine negatives
        nNeg = len(negatives)
        bg_map = self.getTerms4Genes(
            negatives, evid=evid, include_more_general=include_more_general
        )
        ...
        for t in term_set:
            fg_hit = fg_list.count(
                t
            )  # number of foreground genes WITH GO term (number of terms in the list for the collective set of foreground genes)
            bg_hit = bg_list.count(
                t
            )  # number of background genes WITH GO term (number of terms in the list for the collective set of background genes)
            fg_nohit = (
                nPos - fg_hit
            )  # total number of genes in foreground minus that number of hits
            bg_nohit = (
                nNeg - bg_hit
            )  # total number of genes in background minus that number of hits
            pvalue = stats.getFETpval(
                fg_hit, bg_hit, fg_nohit, bg_nohit, False
            )  # one-tailed FET
...
class BinGO:
    ...
    def _decodeGeneIDs(self, gene_codes):
        if (
            type(gene_codes) != list
            and type(gene_codes) != set
            and type(gene_codes) != tuple
        ):
    ...
    def _encodeGeneIDs(self, gene_names):
        if (
            ...
            and type(gene_names) != set
            and type(gene_names) != tuple
        ):
    ...
    def _getGeneEntry(self, gene):
        ...
        for pos in range(0, len(buf) - 1, calcsize("?BI")):
            (qual_bool, evid_int, term_int) = unpack(
                "?BI", buf[pos : pos + calcsize("?BI")]
            )
        ...
        for i in ids:
            ...
            try:
                ...
                for term, evid_and_qual in list(terms.items()):
                    if (
                        evid_and_qual[1] and not evid
                    ):  # if True and no evidence is specified
                        direct.add(term)
    ...
    def getGenes(
        self, terms, evid=None, taxa=None, rel=None, include_more_specific=True
    ):
    ...
    def _readBitFile(self, filename, taxa, termnames=False):
        ...
        while f:
            if not ngene_code:
                ...
                try:
                    if (
                        current_gene_cnt < total_gene_cnt
                    ):  # we are reading gene:terms annotations
                        peek = f.tell()
                        ...
                        current_terms_cnt < total_terms_cnt
                    ):  # we are reading term definitions (term is_a term, term, term, ...)
        ...
        print(
            (
                "Read %d genes and %d term definitions"
                % (current_gene_cnt, current_terms_cnt)
            )
        )
        print(
            ("Annotations start at", annot_offset, "\nDefinitions start at", obo_offset)
        )
        return f

    # FIXME: write code to perform test of taxa enrichment

    def getGOReport_byScore(
        self,
        gene_score_map,
        negatives_score_map={},
        include_more_general=True,
        ...
        """
        fg_map = self.getTerms(
            list(gene_score_map.keys()), include_more_general=include_more_general
        )
        fg_list = []
        for id in fg_map:
            for t in fg_map[id]:
        ...
        if len(negatives_score_map) > 0:
            bg_map = self.getTerms(
                list(negatives_score_map.keys()),
                include_more_general=include_more_general,
            )
        for t in term_set:
            pos = []
            neg = []
        ...
        sorted_pval = sorted(
            list(term_pval.items()), key=lambda v: v[1][0], reverse=False
        )
    ...
    def getGOReport(
        self, positives, background=None, taxa=None, include_more_general=True
    ):
        ...
        if background == None:
            ...
            sorted_cnt = sorted(
                list(term_cnt.items()), key=lambda v: v[1], reverse=True
            )
...
def decode(code, encode_strings):
    ...
    try:
        for pos in range(1, npos):
            accum[pos] = accum[pos - 1] * len(encode_strings[pos - 1])
        indices = [-1 for _ in range(npos)]
        for pos in range(
            npos - 1, -1, -1
        ):  # go backwards, start at last (most significant) position
            indices[pos] = code / accum[pos]
            code -= accum[pos] * indices[pos]
        string = [
            encode_strings[pos][indices[pos]] for pos in range(len(encode_strings))
        ]
    except IndexError as e:
        print((e, code))
    return string
...
def readOBOFile(obofile):
    ...
    for line in src:
        ...
        if line.startswith("[Term]"):
            if (
                in_term_def
            ):  # already defining one, stash it before moving on to the next...
...
def writeBitFile(annotFile, obofile, destFile, taxas=None):
    ...
    gene_index = [
        {} for _ in range(6)
    ]  # count different characters in different positions
    term_index = [
        {} for _ in range(7)
    ]  # count different characters in different positions
    evid_index = {}
    ...
    for line in src:
        ...
        if not (
            taxas and ((taxa == taxas) or (taxa in taxas))
        ):  # The gene does NOT belong to a nominated taxon
            continue
    ...
    print(
        (
            "Wrote header %d\t%d\t%d\t%d\t%d, now at @%d"
            % (
                gene_cnt,
                len(terms),
                len(gene_code),
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: godata <COMMAND> where <COMMAND> is one of the following",
