...
import binfpy.seqdata as seqdata
import sys
...
if len(sys.argv) < 3:
    print(
        "Usage: getclosestgene <chip-peak-bed-file> <reference-genome-annotation-file> [--dist <promoter region bp, default 300bp>]"
    )
...
for site in sites:
    geneInfo = genes.closest(
        site
    )  # finds the closest gene to the binding site, returns a tuple
    ...
    gene = geneInfo[1]
    if (gene.strand == "+" and gene.chromStart >= site.chromStart) or (
        gene.strand == "-" and gene.chromStart <= site.chromStart
    ):
        entry = (
            site.chrom
            + "\t"
