...
class BedEntry:
    ...
    def addOption(
        ...
        if strand:
            self.strand = strand
            self.usestrand = (
                True  # use reverse complement when sequence is requested from genome
            )
        if thickStart != None:
        ...
        if itemRgb != None:
            self.itemRgb = [int(color) for color in itemRgb.split(",")]
        if blockCount != None:
            if blockCount > 0:
                blockSizes = [
                    int(sizeword) for sizeword in blockSizes.rstrip(",").split(",")
                ]
                blockStarts = [
                    int(startword) for startword in blockStarts.rstrip(",").split(",")
                ]
                if len(blockSizes) != blockCount or len(blockStarts) != blockCount:
                    raise RuntimeError(
                        'Blockcount is incorrect in BED entry "%s"' % str(self)
                    )
    ...
    def addBlock(self, relative_start, size):
        ...
        self.blocks.put(
            ival.Interval(
                self.chromStart + relative_start,
...
def dist(entry1, entry2, signed=False, centre2centre=False):
    ...
    if isinstance(entry1, BedEntry) and isinstance(entry2, BedEntry):
        if entry1.chrom == entry2.chrom:
            return ival.dist(
                entry1.getInterval(), entry2.getInterval(), signed, centre2centre
            )
    return None
        ...
        try:
            ...
            if format.lower().startswith("opt") or format.lower().startswith("bed12"):
                if len(words) >= 12:
                    ...
                    entry.addOption(
                        name=words[3], score=float(words[4]), strand=words[5]
                    )
                elif len(words) >= 5:
                ...
                entry.addOption(
                    summit=int(words[1]) - entry.chromStart,
                    tags=int(words[4]),
                    bg=int(words[5]),
            ...
            if not acceptHeaderRows:
                raise RuntimeError(
                    "Error in BED file at row %d (%s)" % (row, e.strerror)
                )
            else:
                headerRow = words
                acceptHeaderRows -= (
                    1  # count down the number of header rows that can occur
                )
    f.close()
    return chroms
        ...
        if row.blocks:  # BED12
            ...
            if row.itemRgb:
                if len(row.itemRgb) == 3:
                    f.write(
                        "%d,%d,%d\t" % (row.itemRgb[0], row.itemRgb[1], row.itemRgb[2])
                    )
                else:
                    f.write("0\t")
            else:
            ...
            if format == "Peaks":
                ...
                f.write(
                    "%s\t%d\t%d"
                    % (row.chrom, row.chromStart, row.chromEnd, row.strand, row.name)
                )
            else:
                f.write(
                    "%s\t%d\t%d\t%s\t%d\t%s"
